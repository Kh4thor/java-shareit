...
public class ItemController {
	...
	@PostMapping
	public Item createItem(@RequestHeader("X-Sharer-User-Id") Long userId, ItemDto itemDto) {
		log.info("Начато создание предмета. Получен объект:" + itemDto + " и id-пользователя=" + userId);
		return itemService.createItem(itemDto, userId);
	}

	@PatchMapping("/{itemId}")
	public Item updateItem(@RequestHeader("X-Sharer-User-Id") Long userId, ItemDto itemDto) {
